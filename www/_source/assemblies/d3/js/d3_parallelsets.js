...
// https://observablehq.com/@john-guerra/parallel-sets



function D3_ParallelSets(vSelector, aProps)
{
	var sSvg = d3.select(vSelector);
	if (sSvg.empty() == true)
	{
		console.log('D3_ParallelSets: Could not identify element based on selector: '+vSelector);
		return;	
	}

	var aDimensions = aProps['dimensions'];
	if (aDimensions.length == 0)
	{
		console.log('D3_ParallelSets: No dimensions are defined.');
		return;	
	}
	...
	if (aData.length == 0)
	{
		console.log('D3_ParallelSets: No data to display.');
		return;	
	}

	var fTension = GetNumberValue(aProps['tension']);
	if (fTension == 0)
	{
		fTension = 1;
	}
	if (fTension < 0.01 || fTension > 1.0)
	{
		console.log('D3_ParallelSets: tension must be between 0.01 and 1.');
		return;	
	}
	
	var strValueAttribute = GetStringValue(aProps['valueattribute']);
	...
	var D3_ParallelSets_FormatNumberValue = function(value)
		{
	...
	d3.parsets = function()
	{
	  var event = d3.dispatch("sortDimensions", "sortCategories"),
	      ...
	      categoryTooltip = D3_ParallelSets_defaultCategoryTooltip,
	      value_ = '', // add = 1
	      spacing = 20,
	      width,
	      height,
	            ...
	            .each(function(d) {
	              ...
	              for (var i = 0; i < n; i++) {
	                ...
	                node = node.children.hasOwnProperty(p) ? node.children[p]
	                    : node.children[p] = {children: {}, value: 0};
	              }
	 ...
	 */
	  ...
	  function layout(tree, dimensions, ordinal)
	  {
	    ...
	    // Stack the counts.
	    dimensions.forEach(function(d) {
	      d.categories = d.categories.filter(function(d) { return d.value; });
	      var x = 0,
	          p = spacing / (d.categories.length - 1);
	      d.categories.forEach(function(c) {
	        c.x = x;
	        c.dx = c.value / total * (width - spacing);
	        c.in = {dx: 0};
	        c.out = {dx: 0};
	        x += c.dx + p;
	      });
	...
	function buildTree(root, data, dimensions, value)
	{
	  ...
	  {
	    var d = data[i];
	    var v = (value == '')?(1):(GetNumberValue(d[value]));
	    var c = 1;
	    var node = root;
	    for (var j = 0; j < nd; j++)
	    {
